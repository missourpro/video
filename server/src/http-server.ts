...
export default class HttpServer {
  ...
  stop(){
    this.server.close(()=>{
      this.server=HttpServer.TO_BE_GARBAGE_COLLECTED;
    });
  }
  private startListening() {
    this.server=this.app.listen(HttpServer.PORT, function () {
      console.log('Server listening on port ' + HttpServer.PORT );
    });
  }
  ...
  private usePlugins() {
    ...
    this.app.use(express.static(HttpServer.PUBLIC_PATH));
  }
}
